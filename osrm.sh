...
nmax=${4:-3000} # maximum batch size for OD table
...
mkdir -p $osm_dir
# geoid=$(cut -d '-' -f 1 <<< "$rgn")
pbf_file=$osm_dir/$city.osm.pbf
if ! [ -f $pbf_file ]; then
    # fname="${city/"$geoid-"/""}-latest.osm.pbf"
    fname="$city-latest.osm.pbf"
...
time python osrm.py -r $city -m $mode -p $port -n $nmax $args
