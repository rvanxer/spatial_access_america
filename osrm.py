...
#%% Main routine
def main(city, scale, mode, nmax, ip, year=2020,
    ...
    None
    """
    city, mode, scale = [x.lower() for x in [city, mode, scale]]
    assert mode in ['drive', 'bike', 'walk'], mode
    assert year in [2010, 2020], year
    label = f'{scale}_{mode}_{year}'
    outpath = Path(outdir) / city / label / '.parquet'
    pts = pd.read_parquet('data/zones.parquet')
    if outpath.exists() and not overwrite:
        print(f'Skipping {city} {label}')
        return
    tstart = dt.now()
    print(f'{tstart}: Processing {city} {label}')
    pts = pts[pts['scale'] == scale]
...
if __name__ == '__main__':
    ...
    for city in kw.regions.split('_'):
        for scale in kw.scales.split('-'):
            print(city, scale, kw.mode, kw.nmax, kw.year, ip)
            try:
                main(city, scale, kw.mode, kw.nmax, year=kw.year,
                    ip=ip, outdir='dist', overwrite=kw.overwrite)
            except Exception as e:
                print('ERROR:', city, scale, kw.mode, kw.year, e)
